...
from app.schemas.dashboard.advertiser import AdvertiserDashboardResponse
...
class AdvertiserDashboardService:
    ...
    async def get_dashboard(self, advertiser_id: int) -> AdvertiserDashboardResponse:
        """
        광고주 대시보드 통합 조회
        ...
        Returns:
            AdvertiserDashboardResponse: 통계 + 최근 추적 현황 5건
        """
        # 매핑된 업체 수
        mapped_count_stmt = select(func.count(AgencyAdvertiserMapping.agency_id)).where(
            AgencyAdvertiserMapping.advertiser_id == advertiser_id
        )
        mapped_result = await self._db.execute(mapped_count_stmt)
        mapped_agency_count = mapped_result.scalar_one()

        # 활성 추적 수
        active_tracking_stmt = select(func.count(RankTracking.id)).where(
            RankTracking.advertiser_id == advertiser_id,
        ...
        blog_posting_stmt = select(func.count(BlogPosting.id)).where(
            BlogPosting.advertiser_id == advertiser_id
        )
        ...
        return AdvertiserDashboardResponse(
            mapped_agency_count=mapped_agency_count,
